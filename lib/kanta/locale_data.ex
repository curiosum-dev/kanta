defmodule Kanta.LocaleData do
  ...
  # The pre-compiled, static map, now with fallback logic applied during generation.
  @locale_data %{
  ...
  @locale_metadata Map.new(@locale_data, fn {locale, {language_name, unicode_flag, flag_colors}} ->
                     {locale,
                      %Metadata{
  ...
  Returns a `%LocaleData{}` struct on success, or `nil` if the locale
  is not found or the input is invalid.

  ## Examples

      iex> LocaleData.get_locale_metadata("pt-BR")
      %LocaleData{
      ...
      iex> LocaleData.get_locale_metadata("pl") # Polish base code
      %LocaleData{
      ...
      iex> LocaleData.get_locale_metadata("pl-pl") # Polish regional code
      %LocaleData{
        language_name: "Polish",
        unicode_flag: "🇵🇱",
      ...
      iex> LocaleData.get_locale_metadata("en") # English base code (no fallback)
      %LocaleData{language_name: "English", unicode_flag: nil, flag_colors: nil}

      iex> LocaleData.get_locale_metadata("xx-yy")
      nil

      iex> LocaleData.get_locale_metadata(123)
      nil
  """
  @spec get_locale_metadata(locale_string :: String.t() | any()) :: Metadata.t() | nil
  def get_locale_metadata(locale_string) when is_binary(locale_string) do
    normalized_locale = String.downcase(locale_string)
    # Simple lookup in the pre-compiled static map
    Map.get(@locale_metadata, normalized_locale)
  end

  def get_locale_metadata(_) do
